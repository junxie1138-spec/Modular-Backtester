...
- It additionally runs an **auto-discovery** pass at import time that registers every generated strategy. Discovery:
  ...
  5. Wraps each per-file import in `try/except`: a single broken generated module is logged and skipped, never aborting the whole registry import. (Generated files are written only after static + functional validation, so this is a safety net, not the common path.)
...
- **`bootstrap()`** — one-time, idempotent. Ensures the `factory-pool` branch exists (creates it off `master` if absent, locally and on the remote). Ensures local-scratch directories are gitignored (`factory/data/_tmp/`, `factory/logs/`, `output/runs/`). Folds any pre-existing single-file stores into this machine's shards: `factory/data/results.json` → `results/<node_id>.jsonl`, `factory/data/dedup_log.txt` → `dedup/<node_id>.txt` (verbatim copy; legacy un-timestamped dedup lines are tolerated per §5). Safe to run repeatedly.
- **`sync_pull()`** — called at cycle start. Ensures the working tree is on `factory-pool`, then `git fetch` + `git pull --rebase`. Brings in other machines' strategies, configs, and result/dedup shards.
- **`sync_push()`** — called at cycle end. `git add` this cycle's new strategy file, config file, and this machine's two shards; `git commit`; `git push`. On a non-fast-forward rejection (another machine pushed first), `git pull --rebase` and retry, bounded by `push_retries` (default 5). Because every machine writes only its own shards and uniquely-named files, the rebase is always conflict-free, so retries converge immediately.
...
5. `append_registry_entry` is gone; `backtester/strategies/registry.py` auto-discovers `gen_*.py` only, in sorted order, after `importlib.invalidate_caches()`, skipping (and logging) any module that fails to import; curated strategies remain registered.
6. `factory/sync.py` provides `bootstrap`, `sync_pull`, `sync_push`; `sync_push` retries a non-fast-forward up to `push_retries` times and converges; all sync failures are logged and never abort the loop.
